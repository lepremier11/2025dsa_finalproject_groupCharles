...
# Set theme for publication-ready plots
theme_pub <- theme_bw(base_size = 14) +
  theme(panel.grid = element_blank(),
        strip.background = element_rect(fill = "#f0f0f0"),
        legend.position = "top")

# Nutrients to explore visually
nutrients <- c("pH", "SM", "EC", "Ca", "Mg", "Mn", "Zn", "N", "P", "K")

# Plot nutrient distributions by amendment and location
for (nutrient in nutrients) {
  p <- ggplot(data, aes_string(x = "ammendment", y = nutrient, fill = "location")) +
    ...
    labs(title = paste("Distribution of", nutrient, "by Amendment and Location"),
...
# Define analysis wrapper function
analyze_nutrient <- function(response) {
  message("\n\n===== Analyzing: ", response, " =====")}
```
As described earlier, a LME model with an autoregressive order 1 (AR1) to account for irregular intervals between sampling points.
```{r}
# Overall model
  formula_all <- as.formula(paste(response, "~ day * ammendment * location"))
  model_all <- lme(formula_all, data = data, random = ~1 | rep_am, correlation = corAR1(form = ~1 | rep_am))
  print(anova(model_all))
# LSMeans
  em_all <- emmeans(model_all, ~ ammendment * location | day)
  cld_all <- cld(em_all, reversed = TRUE, adjust = "Bonferroni", Letters = letters)
  print(cld_all)

  # Pairwise comparisons
  print(summary(contrast(em_all, method = "pairwise", adjust = "tukey")))
```
```{r}
# FL model
  formula_fl <- as.formula(paste(response, "~ ammendment + day + ammendment * day"))
  model_fl <- lme(formula_fl, data = data_fl, random = ~1 | rep_am, correlation = corAR1(form = ~1 | rep_am))
  message("\n-- FL Model --")
  print(anova(model_fl))
  print(summary(emmeans(model_fl, ~ ammendment)))
  print(summary(contrast(emmeans(model_fl, ~ ammendment), method = "pairwise", adjust = "tukey")))
  print(summary(contrast(emmeans(model_fl, ~ ammendment * day), method = "pairwise", adjust = "tukey")))

  # GA model
  formula_ga <- as.formula(paste(response, "~ ammendment + day + ammendment * day"))
  model_ga <- lme(formula_ga, data = data_ga, random = ~1 | rep_am, correlation = corAR1(form = ~1 | rep_am))
  message("\n-- GA Model --")
  print(anova(model_ga))
  print(summary(emmeans(model_ga, ~ ammendment)))
  print(summary(contrast(emmeans(model_ga, ~ ammendment), method = "pairwise", adjust = "tukey")))
  print(summary(contrast(emmeans(model_ga, ~ day * ammendment), method = "pairwise", adjust = "tukey")))
}

# Run analysis
for (nutrient in nutrients) {
  analyze_nutrient(nutrient)
}
```
